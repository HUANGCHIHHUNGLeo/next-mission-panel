...
export default function LoginPage() {
  ...
  const handleModeSwitch = (adminMode) => {
    ...
    setIsLogin(true); // 切換到管理員模式時預設為登入
    setEmail('');
    setPassword('');
    setDisplayName('');
    setPhone('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        if (data.user) {
          ...
          if (isAdminMode) {
            ...
            if (userError || !userData) {
              ...
              throw new Error('無法查詢用戶資料，請稍後再試');
            }
        ...
        // 註冊邏輯 - 只有學生模式才能註冊
        if (isAdminMode) {
          throw new Error('管理員帳號無法透過此頁面註冊');
        }
      ...
      <div className="bg-gray-800/90 backdrop-blur-sm p-8 rounded-2xl shadow-2xl w-full max-w-md border border-gray-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <input
              type="email"
              placeholder={isAdminMode ? "管理員 Email *" : "Email *"}
              value={email}
          ...
          <div>
            <input
              ...
              required
            />
            {password && passwordCheck.message && !isAdminMode && (
          ...
          <button
            type="submit"
            disabled={loading || (!isLogin && !isAdminMode && (!passwordCheck.valid || !phoneValid || !ageValid || !privacyConsent))}
            className={`w-full text-white py-3 rounded-lg font-semibold transition-all duration-200 disabled:cursor-not-allowed ${
              isAdminMode 
                ? 'bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 disabled:from-gray-600 disabled:to-gray-600'
                : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:from-gray-600 disabled:to-gray-600'
            }`}
          >
            {loading ? (
